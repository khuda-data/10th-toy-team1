...
| [`scripts/`](scripts/) | 유틸 스크립트 (논문 그림 추출 등) — 재생성 가능 | 해설 그림 작업 시 |
| `src/` *(예정)* | 공용 코드 — 모듈 폴더마다 주인 1명 | 코드 작업 시작 시 |
| `sandbox/<영문이름>/` *(예정)* | 개인 실험 공간 — 리뷰 없이 자유 푸쉬 | 각자 실험할 때 |
| `data/` · `notebooks/` *(예정)* | 데이터·분석 노트북 | 데이터 작업 시작 시 |
