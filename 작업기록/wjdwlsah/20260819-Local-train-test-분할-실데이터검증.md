...
- 브랜치: feat/wjdwlsah-local-split-verification (`feat/wjdwlsah-global-split-verification`에서 분기 — 그 브랜치의 Windows 크래시 수정이 있어야 파이프라인이 끝까지 돎)
- **PR 순서**: `feat/wjdwlsah-global-split-verification` → 이 브랜치 순으로 리뷰·머지 부탁드립니다(오버샘플링 완화 때 PR #4→PR #5 순서와 동일한 방식).

## 지침 (내가 시킨 것)
- Global Model 데이터셋을 §15 규칙(80:20)대로 먼저 train/test로 나눈 뒤(별도 PR), Local Model 6개 직군 데이터셋도 같은 비율로 train/test로 나누되 **Local train은 전부 Global train에만, Local test는 전부 Global test에만 속하도록**(Global test가 Local train에 새어 들어가는 일이 없도록) 만들어서 독립적인 PR로 올려달라는 지시.
