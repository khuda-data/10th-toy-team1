...
1. **`major_group`(전공계열) 결측 사유** — 결측 25.16%(2026-08-16 재확인)를 전부 `Missing`으로 분류했는데, 전공 없는 사람(비대학 진학자 등)이 있다면 `NotApplicable`이어야 할 수도 있음. 코드북에 `a413`이 `[공통]`(전원 대상) 태그만 있어 확정 못 함.
