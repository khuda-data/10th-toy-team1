...
현재 Global 모델링은 [14번 단계별 실행 흐름](../../plan/details/14-Global-모델링-단계별-실행흐름.md)의 1차·2차 비교를 먼저 수행하므로 `model_config.yaml`의 `official_global_models`에 등록된 Logistic Regression과 XGBoost만 사용한다. 다른 모델은 공통 모듈이 지원하더라도 이 Global 선택 단계에 섞지 않는다.
...
| Global LR/XGBoost 단계별 실행 순서 | **사람(Kim ByungKyu)이 직접 지시한 2026-08-20 모델링 흐름** | ✅ 2026-08-20 Kim ByungKyu |
